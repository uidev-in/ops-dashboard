function myfunc(){
  ...
  var progress_bar = 0;
  var work_yes_no = $('input:radio[name="work_again"]').is(':checked');
  var star_1 = $('input[name="quality_of_work"]').is(":checked");
  var star_2 = $('input[name="timeliness_of_vendor"]').is(":checked");
  var star_3 = $('input[name="communication_skills_of_vendor"]').is(":checked");
  var star_4 = $('input[name="involvement_of_vendor"]').is(":checked");
  var star_5 = $('input[name="overall_experience_of_vendor"]').is(":checked");
  var star_6 = $('input[name="overall_experience_of_exmyb_and_kam"]').is(":checked");
  var star_7 = $('input[name="recommend_exmyb"]').is(":checked");
  var textarea = $('textarea').val().trim().length;
  if(textarea){
    progress_bar += 10;
  }
  ...
  var progressMessage = $(".progress-top");
  var temp = "calc(" + progress_bar + " - 18px)";
  
  progress.css("width", progress_bar);
  progressMessage.text(progress_bar);
  progressMessage.css("left", temp);

}

$(function(){

    $('input:radio[name="work_again"]').change(function() {
          myfunc();
    });
    $('input[type=text]').on('change keyup keydown',function(){
          myfunc();
    });
    // $('input[name="star-*"]').change(function() {
    //       alert("sdwfd");
    // });
    $('input[name="quality_of_work"]').on('change keyup keydown',function(){
          // console.log($(this).val());
          myfunc();
    });
    $('input[name="timeliness_of_vendor"]').on('change keyup keydown',function(){
      myfunc();
    });
    $('input[name="communication_skills_of_vendor"]').on('change keyup keydown',function(){
      myfunc();
    });
    $('input[name="involvement_of_vendor"]').on('change keyup keydown',function(){
      myfunc();
    });
    $('input[name="overall_experience_of_vendor"]').on('change keyup keydown',function(){
      myfunc();
    });
    $('input[name="overall_experience_of_exmyb_and_kam"]').on('change keyup keydown',function(){
      myfunc();
    });
    $('input[name="recommend_exmyb"]').on('change keyup keydown',function(){
      myfunc();
    });
    ...
    $('textarea').on('change keyup keydown',function(){
         ...
         if(startyear == parseInt($('.mpr-calendar:first h5 span').html()))
            $('.mpr-calendar:first .mpr-selected:first').css("background","#82C424");
          if(endyear == parseInt($('.mpr-calendar:last h5 span').html()))
            $('.mpr-calendar:last .mpr-selected:last').css("background","#82C424");
         }
